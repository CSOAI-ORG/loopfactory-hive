...
```
L7  PRESENTATION    automation green + workflow blue (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      cron-ai-mcp, webhook-ai-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: automation workflows, triggers, actions, integrations
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "loopfactory"
...
```bash
git clone https://github.com/CSOAI-ORG/loopfactory-hive
cd loopfactory-hive
...
`https://loopfactory.ai/.well-known/agent-card.json` — see `agent-card.json`.
